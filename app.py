...
prop_col = ['address', 'water', 'council', 'strata', 'estimated full rent', 'estimated partial rent']
...
property_table = dash_table.DataTable(
    ...
    columns = (
        [{'id':c, 'name':c} for c in prop_col ]
    ),
    data=[
        dict(Model=i, **{param: None for param in prop_col})
        for i in range(1, 5)
    ],
    editable=True,
    row_deletable=True,
...
loan_table = dash_table.DataTable(
    ...
    columns = (
        [{'id':c, 'name':c} for c in loan_col ]
    ),
    data=[
        dict(Model=i, **{param: None for param in loan_col})
        for i in range(1, 5)
    ],
    editable=True,
    row_deletable=True,
...
app.layout = [
    html.H1(children='Investment and principal residence return calculator', style={'textAlign':'center'}),
    dcc.Input(
        id="input_deposit",
        type='number',
